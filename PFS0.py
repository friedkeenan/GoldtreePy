...
class PFS0:
    ...
    def __init__(self,filename):
        ...
        len_strings=struct.unpack("=I",self.read_raw(0x8,0xC))[0]
        self.files=[PFS0.FileEntry(self.read_raw(0x10+0x18*x,0x10+0x18*(x+1))) for x in range(num_files)]
        self.header_size=0x10+0x18*num_files
        file_names=self.read_raw(self.header_size,self.header_size+len_strings).split(b"\0")[:num_files]
    ...
    def read_chunks(self,idx,chunk_size=0x100000):
        ...
        while to_read>0:
            ...
            yield self.read_raw(cur_offset,cur_offset+tor)
            cur_offset+=tor
            to_read-=tor
